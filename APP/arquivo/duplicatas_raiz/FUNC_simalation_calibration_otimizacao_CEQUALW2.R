...
Simula_Quali<-function(diretorio_cequal, dados_simulacao, parametros){
  ...
  #diretorio_cequal="C:\\Users\\daniel.cid\\OneDrive\\Funceme\\Qualidade_da_agua\\Acoplamento SwatCequal\\Reservatorios\\EdsonQueiroz\\EDQ_CEQUAL_2016_2020"
  dir=diretorio_cequal
  julian_day_ini=dados_simulacao$julian_day_ini
  julian_day_fim=dados_simulacao$julian_day_fim
  ano_simul=dados_simulacao$ano_simul
  nsegmentos=dados_simulacao$nsegmentos
  seg=dados_simulacao$seg
  preproc=dados_simulacao$preproc
  ctmax=dados_simulacao$ctmax
  ctver=dados_simulacao$ctver
  ctmin=dados_simulacao$ctmin
  res=dados_simulacao$res
  cota_obs=dados_simulacao$cota_obs
  temp_obs=dados_simulacao$temp_obs
  evap_obs=dados_simulacao$evap_obs
  fosfato_obs=dados_simulacao$fosfato_obs
  nitrito_obs=dados_simulacao$nitrito_obs
  chla_obs=dados_simulacao$chla_obs
  do_obs=dados_simulacao$do_obs
  ELWS=dados_simulacao$ELWS
  TEMP=dados_simulacao$TEMP
  tipo=dados_simulacao$tipo
  PO4=dados_simulacao$PO4
  DO=dados_simulacao$DO
 
  ALG1=dados_simulacao$ALG1
...
###FAER VERIFICACAO DE ACORDO COM O TIPO
  ...
  controle <- readLines(arquivo_controle, warn = FALSE)
  ...
  bth_linhas<-readLines(paste0(dir,"/",arquivo_bth), warn=FALSE)
  bth_linhas2 <- substitui_bloco(bth_linhas, "ELWS", ELWS)
  novo_arquivo_bth<-paste0("bath",ano_simul,"-",julian_day_ini,".prn")
  writeLines(bth_linhas2, paste0(dir,"/",novo_arquivo_bth), sep = "\n")
...
#  writeLines(controle, file.path(diretorio_cequal,"w2_con.npt"), sep = "\n")
  ...
  if (tipo == 1 || tipo == 2){
    ...
    ## Arquivo WSC
    wsc=readLines(paste0(dir,"/wscR.prn"))
    ...
    writeLines(wsc, file.path(diretorio_cequal,"wsc.prn"), sep = "\n")
    ...
    write(cab1, file.path(diretorio_cequal,"shade.prn"))
    write(cab2, file.path(diretorio_cequal,"shade.prn"), append = T)
    write.fwf(
      as.data.frame(dado_entrada), 
      file.path(diretorio_cequal,"shade.prn"),
      sep="",
      append=T,
      colnames = F,
      rownames=F,
...
do_obs=dados_simulacao$do_obs
  ...
  linhas <- readLines(arquivo_c, warn = FALSE) 
  ...
  dados_flux=suppressWarnings(read_csv(arquivo_f, show_col_types = FALSE))
...
Otmiza_Quali <- function(x, contexto, diretorio_cequal, varsobj, objfun, windows=FALSE){
  ...
  bat_pre <- c(
    ...
    paste0("\"",getwd(), "/W2Pre3.7.exe","\"")
  )
  
  writeLines(bat_pre, arq_exe1)
  
  # Conteúdo do BAT principal
  bat_run <- c(
    "@echo off",
    paste0("cd /d \"", diretorio_cequal, "\""),
    paste0("\"",getwd(), "/w2_3.7_64.exe","\"")
  )
  writeLines(bat_run, arq_exe2)
  ...
  resultado_simulacao=Simula_Quali2(
    simul=1, 
    diretorio_cequal=diretorio_cequal, 
    dados_simulacao=contexto$dados_simulacao, 
    preproc=preproc, salvar_figura=0,
    salvar_resultados=0,
    simul_aleatoria=1,
    varsobj = varsobj,
    objfun = objfun)
  ...
  metricas <- resultado_simulacao$metricas
    ...
    if (nrow(contexto$dados_simulacao$dados_janela) <= 1) {
    ...
    erro_janela <- vector("list", nrow(contexto$dados_simulacao$dados_janela))

    for (sim_j in 1:(nrow(contexto$dados_simulacao$dados_janela)-1)){
      ...
      #print(contexto$dados_simulacao_janela)
      gera_arquivo<-Simula_Quali(diretorio_cequal,dados_simulacao=dados_simulacao_janela, parametros = x)
      ...
      metricas<- Simula_Quali2(simul=sim_j,diretorio_cequal=diretorio_cequal, dados_simulacao=dados_simulacao_janela, preproc=preproc, salvar_figura=0,salvar_resultados=0,simul_aleatoria=0 )
...
carregar_dados_simulacao <- function(diretorio_cequal,dados_sim) {
  ...
  linhas <- readLines(arquivo_controle, warn = FALSE)
  ...
  dados_obs <- readxl::read_excel(caminho_arquivo, sheet = nome_aba)
...
# 
# dir <- "C:/Users/daniel.cid/OneDrive/Funceme/Qualidade_da_agua/Acoplamento SwatCequal/Reservatorios/EdsonQueiroz/EDQ_CEQUAL_2016_2020/Resultados_tipo1"
# metricas_consolidadas <- readRDS(file.path(dir, "_tudo_junto.rds"))
# metricas_consolidadas <- bind_rows(metricas_consolidadas, .id = "simulacao")
...
montar_preview_cenario_afluencia <- function(
  ...
  linhas <- readLines(arquivo_controle_base, warn = FALSE)
...
preparar_cenario_afluencia_cequal <- function(
  ...
  ok_copy <- file.copy(arquivo_controle_base, arquivo_controle_R, overwrite = TRUE)
  if (!ok_copy) {
    stop("Não foi possível copiar o arquivo de controle para a pasta do cenário.")
  }

  linhas <- readLines(arquivo_controle_R, warn = FALSE)
  ...
  writeLines(linhas, arquivo_controle_cenario, sep = "\n")
...
executar_cenario_afluencia_cequal <- function(
  ...
  exe_pre <- file.path(app_dir, "W2Pre3.7.exe")
  exe_run <- file.path(app_dir, "w2_3.7_64.exe")
  ...
  status_run <- shell(shQuote(arq_exe2), wait = TRUE)
 ...
 # metricas<- Simula_Quali2(simul=i,diretorio_cequal=diretorio_resultados, dados_simulacao=dados_simulacao, preproc=preproc, salvar_figura=1,salvar_resultados=1,simul_aleatoria=0 )
  ...
  if (length(arquivos_resultado) > 0) {
    copias_resultado <- file.copy(
      from = arquivos_resultado,
      to = diretorio_resultados,
      overwrite = TRUE,
      copy.mode = TRUE,
      copy.date = TRUE
    )
  ...
  linhas_controle <- readLines(arquivo_controle_raiz, warn = FALSE)
...
ler_tsr_resultado <- function(arquivo_tsr) {
  txt <- readLines(arquivo_tsr, warn = FALSE)
  txt <- stringr::str_replace_all(
    txt,
    "([0-9]E[-+][0-9]+)-",
    "\\1 -"
...
gerar_figura_cenario_qualidade <- function(diretorio_resultados,ano_simul,nome_saida = "cenario_qualidade_reservatorio.png") {
  ...
  linhas_controle <- readLines(arquivo_controle, warn = FALSE)
