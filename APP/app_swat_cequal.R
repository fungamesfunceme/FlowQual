...
dir_engenharia_reversa <- file.path(app_dir, "R", "engenharia_reversa")
source(file.path(dir_engenharia_reversa, "core.R"), local = TRUE)
source(file.path(dir_engenharia_reversa, "dados.R"), local = TRUE)
source(file.path(dir_engenharia_reversa, "api.R"), local = TRUE)
source(file.path(dir_engenharia_reversa, "cequal_exportacao.R"), local = TRUE)
source(file.path(dir_engenharia_reversa, "shiny.R"), local = TRUE)
source(file.path(app_dir, "R", "io_seguro.R"), local = TRUE)

localizar_script_app <- function(nome_arquivo) {
  caminho <- file.path(app_dir, "R", nome_arquivo)
  if (!file.exists(caminho)) {
    stop(
      "O script ", nome_arquivo,
      " nao foi encontrado na subpasta R do aplicativo.",
      call. = FALSE
    )
  }
  caminho
...
source(localizar_script_app("ReadFile_Rch_SWAT.R"), local = TRUE)
source(localizar_script_app("ReadFile_cio_SWAT.R"), local = TRUE)
source(localizar_script_app("contexto_simulacao.R"), local = TRUE)
source(
  localizar_script_app("FUNC_simalation_calibration_otimizacao_CEQUALW2.R"),
  local = TRUE
)
          ...
          div(
            ...
            div(
              ...
              div(
                class = "welcome-alert",
                paste(
                  "Para usar SWAT ou CE-QUAL-W2, selecione um reservatório",
                  "no campo ao lado. A Engenharia Reversa possui seleção própria."
                )
              ),
              ...
              div(
                ...
                actionLink(
                  inputId = "card_engenharia_reversa",
                  label = div(
                    class = "module-card module-card-click",
                    h4("Engenharia Reversa"),
                    p(
                      paste(
                        "Cálculo da afluência a partir do comportamento",
                        "observado do reservatório."
                      )
                    )
                  )
                ),
...
server <- function(input, output, session) {
  registrar_servidor_engenharia_reversa(input, output, session, app_dir)
  ...
  observe({
    ...
    if (reservatorio_ok) {
      ...
      if (isolate(input$abas_principais) %in% c("swat", "cequal")) {
        updateTabsetPanel(
          session,
  ...
  tanatos <- "\\tanatos.funceme.br/MODQUALIAGUA/7. ACOPLAMENTO"
  # EDQ_SWAT= "C:\\Users\\daniel.cid\\OneDrive\\Funceme\\Qualidade_da_agua\\2025_1\\SwatCequal\\Reservatorios\\EdsonQueiroz"
  # #outroc="C:/Users/daniel.cid/OneDrive/Funceme/Qualidade_da_agua/2024_2/EDQ_CEQUAL_2016_2020"
  # EDQ_CEQUAL_2016_2020="C:\\Users\\daniel.cid\\OneDrive\\Funceme\\Qualidade_da_agua\\2025_1\\SwatCequal\\Reservatorios\\EdsonQueiroz\\EDQ_CEQUAL_2016_2020"
  # JAB_CEQUAL_2016_2020="C:\\Users\\daniel.cid\\OneDrive\\Funceme\\Qualidade_da_agua\\2025_1\\SwatCequal\\Reservatorios\\JaburuI\\LUCIANA_PROD_3_1B\\Calibracao_Fosforo\\CalP_JAB2016_2020_Bath400m"
  # JAB_SWAT="C:\\Users\\daniel.cid\\OneDrive\\Funceme\\Qualidade_da_agua\\2025_1\\SwatCequal\\Reservatorios\\JaburuI"

  
  #outroc="/Users/danielcid/Library/CloudStorage/OneDrive-Pessoal/Funceme/Qualidade_da_agua/2025_1"
  volumes <- c("Pasta do app"= app_dir, Home = fs::path_home(), "R Installation" = R.home(), getVolumes()(),"tanatos.funceme.br"=tanatos)#, "EDQ_SWAT" = EDQ_SWAT, "EDQ_CEQUAL_2016_2020" = EDQ_CEQUAL_2016_2020, "JAB_SWAT"=JAB_SWAT,"JAB_CEQUAL_2016_2020" = JAB_CEQUAL_2016_2020)
  ...
  observeEvent(input$directory_cequal, {
    ...
    candidatos_seletor <- c(
      file.path(app_dir, "selecionar_arquivo_npt_windows.ps1"),
      file.path(app_dir, "scripts", "selecionar_arquivo_npt_windows.ps1")
    )
    candidatos_seletor <- candidatos_seletor[file.exists(candidatos_seletor)]
    if (length(candidatos_seletor) == 0L) {
      showNotification(
    ...
    script_seletor <- candidatos_seletor[[1L]]
  ...
  observeEvent(input$card_engenharia_reversa, {
    updateTabsetPanel(
      session,
      inputId = "abas_principais",
      selected = "engenharia_reversa"
    )
  })
  ...
  #  req(input$directory_cequal)
    ...
    linhas <- tentar_operacao_arquivo_shiny(
      function() {
        ler_linhas_seguro(
          arquivo_controle,
          warn = FALSE,
          descricao = "arquivo de controle do CE-QUAL-W2"
        )
      },
      caminho = arquivo_controle,
      verbo = "ler",
      session = session
    )
    if (is.null(linhas)) return(NULL)
    ...
    if (!is.na(bth)) {
      ...
      if (file.exists(arquivo_bth)) {
        ...
        linhas_bth <- tentar_operacao_arquivo_shiny(
          function() {
            ler_linhas_seguro(
              arquivo_bth,
              warn = FALSE,
        ...
        if (is.null(linhas_bth)) return(NULL)
  ...
  output$param_filepath <- renderText({
    ...
    param <- tentar_operacao_arquivo_shiny(
      function() {
        ler_linhas_seguro(
          path,
          warn = FALSE,
          descricao = "arquivo de parametros"
        )
      },
      caminho = path,
      verbo = "ler",
  ...
  output$param_table <- renderTable({
     ...
     ext  <- tools::file_ext(input$param_file$name)
      ...
      counts <- sapply(c(comma = ",", semicolon = ";", tab = "\t", space = " "),
                       function(d) {
                         stringr::str_count(first_line, stringr::fixed(d))
                       })
    ...
    sep <- tentar_operacao_arquivo_shiny(
      function() guess_sep(path),
      caminho = path,
      verbo = "ler",
      session = session
    )
    if (is.null(sep)) return(NULL)
    df <- tentar_operacao_arquivo_shiny(
      function() {
        switch(
          ext,
...
observeEvent(input$update, {
  ...
  withProgress(message = 'Processando dados...', value = 0, {
    ...
    arquivo_salvo <- tentar_operacao_arquivo_shiny(
      function() {
        gravar_com_seguranca(
          file_name,
          function(destino) {
            write(cab, destino)
            write("", destino, append = TRUE)
            write.fwf(
              cab2,
              destino,
              sep = "",
          ...
          descricao = "arquivo de entrada do CE-QUAL-W2"
        )
      },
      caminho = file_name,
      verbo = "salvar",
    ...
    if (is.null(arquivo_salvo)) return(NULL)
...
###aqui comeca auxiliar
  ...
  abas_disponiveis <- tentar_operacao_arquivo_shiny(
    function() {
      validar_arquivo_para_leitura(
        caminho_arquivo,
        "arquivo de dados observados"
      )
...
##
  ...
  dados_simulacao <- tentar_operacao_arquivo_shiny(
    function() {
      carregar_dados_simulacao(
        diretorio_cequal,
        dados_sim
      )
    },
...
####### EXECUTAR RUN SIMULAÇÃO
      ...
      observeEvent(input$run_exe, {
        ...
        preproc <- contexto$dados_sim$preproc 
        ...
        executaveis_ok <- tentar_operacao_arquivo_shiny(
          function() {
            validar_arquivo_para_leitura(
              executavel_pre,
              "pre-processador do CE-QUAL-W2"
            )
            validar_arquivo_para_leitura(
              executavel_cequal,
              "executavel do CE-QUAL-W2"
            )
            TRUE
          },
          caminho = executavel_cequal,
          verbo = "localizar",
          session = session
        )
        if (is.null(executaveis_ok)) return(NULL)
    ...
    if (isFALSE(input$use_windows)) {
     ...
     for (i in 1:nrow(param)) {
       ...
       gera_arquivo<-Simula_Quali(diretorio_cequal,dados_simulacao=dados_simulacao, parametros = parametros_simul)  
 
       arq_exe1 <-paste0(diretorio_cequal,"/0_RunW2Pre.Bat")
       arq_exe2 <-paste0(diretorio_cequal,"/0_RunW2.Bat")

       
       bat_pre <- c(
         "@echo off",
         paste0("cd /d \"", diretorio_cequal, "\""),
         paste0("\"", executavel_pre, "\"")
       )
       ...
       bat_run <- c(
         "@echo off",
         paste0("cd /d \"", diretorio_cequal, "\""),
         paste0("\"", executavel_cequal, "\"")
       )
       arquivos_bat_ok <- tentar_operacao_arquivo_shiny(
         function() {
           gravar_linhas_seguro(
             bat_pre,
             arq_exe1,
             descricao = "script de pre-processamento"
           )
           gravar_linhas_seguro(
             bat_run,
             arq_exe2,
             descricao = "script de execucao"
           )
           TRUE
         },
         caminho = arq_exe2,
       ...
       if (is.null(arquivos_bat_ok)) return(NULL)
         ...
         shell(shQuote(arq_exe2), wait = TRUE)
       
   
           ##analiso os resultados, salvo figuras, etc
       metricas<- Simula_Quali2(simul=i,diretorio_cequal=diretorio_cequal, dados_simulacao=dados_simulacao, preproc=preproc, salvar_figura=1,salvar_resultados=1,simul_aleatoria=0 )
     ...
     ####---- fim da simulaçao sem janelas
      ...
      if (nrow(dados_simulacao$dados_janela) <= 1) {
      ...
      #erro_janela<- matrix(NA, nrow=nrow(dados_simulacao$dados_janela), ncol=nobj)
      erro_janela <- vector("list", nrow(dados_simulacao$dados_janela))
      ...
      for (sim_j in 1:(nrow(dados_simulacao$dados_janela)-1)){
        ...
        dados_sim <- contexto_janela$dados_sim
        dados_simulacao_janela <- contexto_janela$dados_simulacao
     
    
       # print(dados_simulacao$julian_day_fim)
        gera_arquivo<-Simula_Quali(diretorio_cequal,dados_simulacao=dados_simulacao_janela, parametros = parametros_simul)
        ...
        metricas <- Simula_Quali2(
          simul = identificador_execucao,
          diretorio_cequal = diretorio_cequal,
          dados_simulacao = dados_simulacao_janela,
          preproc = preproc,
          salvar_figura = 1,
          salvar_resultados = 1,
          simul_aleatoria = 0,
        ...
        arquivo_metricas <- file.path(diretorio_cequal, paste0(
          "/resultados_tipo", input$tipo_simulacao,
          "/Janela/metricas_simul_", i, ".csv"
        ))
        metricas_salvas <- gravar_tabela_segura(
          metricas_consolidadas,
          arquivo_metricas,
          readr::write_delim
        )
        if (!isTRUE(metricas_salvas)) return(NULL)
        
        arquivo_metricas_rds <- file.path(diretorio_cequal, paste0(
            "/resultados_tipo", input$tipo_simulacao,
            "/Janela/metricas_simul_", i, ".rds"
        ))
        rds_salvo <- tentar_operacao_arquivo_shiny(
          function() salvar_rds_seguro(erro_janela, arquivo_metricas_rds),
          caminho = arquivo_metricas_rds,
          verbo = "salvar",
          session = session
        )
        if (is.null(rds_salvo)) return(NULL)
      ...
      arquivo_metricas_param <- file.path(diretorio_cequal, paste0(
        "/resultados_tipo", input$tipo_simulacao,
        "/Janela/metricas_janela_param.csv"
      ))
      metricas_param_salvas <- gravar_tabela_segura(
        metricas_consolidadas,
        arquivo_metricas_param,
        readr::write_delim
      )
      if (!isTRUE(metricas_param_salvas)) return(NULL)
      
      arquivo_metricas_param_rds <- file.path(diretorio_cequal, paste0(
          "/resultados_tipo", input$tipo_simulacao,
          "/Janela/metricas_janela_param.rds"
      ))
      rds_param_salvo <- tentar_operacao_arquivo_shiny(
        function() salvar_rds_seguro(erro_param, arquivo_metricas_param_rds),
        caminho = arquivo_metricas_param_rds,
        verbo = "salvar",
        session = session
      )
      if (is.null(rds_param_salvo)) return(NULL)
...
################## CHAMA FUNÇÃO DE OTIMIZAÇÃO
     ...
     arquivo_pareto <- paste0(
       contexto$diretorio_cequal,
       "/resultados_tipo",
       input$tipo,
       "/pareto_resultados.csv"
     )
     pareto_salvo <- gravar_tabela_segura(
       df_resultados,
       arquivo_pareto,
       utils::write.csv,
       row.names = FALSE
     )
     if (!isTRUE(pareto_salvo)) return(NULL)
     
     arquivo_parametros_otm <- paste0(
       contexto$diretorio_cequal,
       "/resultados_tipo",
       input$tipo,
       "/parametros_otm.csv"
     )
     parametros_salvos <- gravar_tabela_segura(
       df_param,
       arquivo_parametros_otm,
       utils::write.csv,
       row.names = FALSE
     )
     if (!isTRUE(parametros_salvos)) return(NULL)
